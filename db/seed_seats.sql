...
-- Optional: add a uniqueness constraint to prevent future duplicates.
-- ALTER TABLE seat ADD CONSTRAINT seat_unique_per_recital UNIQUE (recital_id, section, row, number);
...
WITH expected AS (
  SELECT SUM(seat_count)::int AS expected_seats
  FROM (
    VALUES
      ('A', 1, 8), ('A', 2, 9), ('A', 3, 10), ('A', 4, 10), ('A', 5, 10),
      ('A', 6, 9), ('A', 7, 9), ('A', 8, 8), ('A', 9, 8), ('A', 10, 7), ('A', 11, 7),
      ...
      ('E', 1, 8), ('E', 2, 9), ('E', 3, 10), ('E', 4, 10), ('E', 5, 10),
      ('E', 6, 9), ('E', 7, 9), ('E', 8, 8), ('E', 9, 8), ('E', 10, 7), ('E', 11, 7)
  ) AS layout(section, row_num, seat_count)
)
SELECT
  r.id AS recital_id,
  r.name AS recital_name,
  COALESCE(COUNT(s.id), 0)::int AS actual_seats,
  e.expected_seats
FROM recital r
LEFT JOIN seat s ON s.recital_id = r.id
CROSS JOIN expected e
GROUP BY r.id, r.name, e.expected_seats
ORDER BY r.id;
